...
//	catch (...)
//	{
//		throw PBXHelperException(_TEXT("create PBObjectFunctionInvoker failed: can't find function"));
//	}
}
...
//	if ((d_session->InitCallInfo(cls, d_mid, &d_ci)) != PBX_OK)
//		throw PBXHelperException(_TEXT("create PBObjectFunctionInvoker failed: can't find function"));
}
...
//	try
//	{
		// First search user defined functions
		if (!bFound)
		{
		...
		if (!bFound)
		{
			...
			d_class = d_session->FindClass(sysGroup, "SystemFunctions");
			if (d_class)
				d_mid = d_session->GetMethodID(d_class, methodName, PBRT_FUNCTION, signature);
...
//	catch (...)
//	{
//		throw PBXHelperException(_TEXT("create PBGlobalFunctionInvoker failed: can't find function"));
//	}
}
...
//	catch (...)
//	{
//		throw PBXHelperException(_TEXT("create PBObjectFunctionInvoker failed: can't find event"));
//	}
}
